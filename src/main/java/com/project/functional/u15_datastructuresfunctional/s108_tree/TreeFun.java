...
public class TreeFun<E extends Comparable<E>> {
    ...
    @SafeVarargs
    public static <E extends Comparable<E>> TreeFun<E> tree(E...e) {
        TreeFun<E> tree = NIL;
        for (int i = 0; i < e.length; i++) {
            tree = tree.insert(e[i]);
        }
        return tree;
    }
    ...
    public TreeFun<E> remove(E element){
        if(element.compareTo(this.value) < 0) {
            ...
            if(this.left.isEmpty())
                ...
                return this.left;
                //having two child nodes; needs to find the node which can replace root
                //either replace it by max node value from left subtree or smallest node from right subtree
            else {
                E max = this.left.max();
                return new TreeFun<E>(this.left.remove(max), max, this.right);
            }
        }
    }
